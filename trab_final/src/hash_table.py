...
# id do usuário com mais avaliações
ID = '130642'
TAMANHO_TABELA = 7993
...
class HashTable:
    ...
    def __str__(self):
        ...
        for i, lista in enumerate(self.table):
            ...
            if lista:
                output += ", ".join([str(_) for _ in lista])
    ...
    def _resize(self):
        ...
        for bucket in self.table:
            for _ in bucket:
                hash_index = self._hash(_.id[0])
                new_table[hash_index].append(_)

        self.table = new_table
        del new_table

    def insert(self, elemento: Jogador | Usuario):
        hash_index = self._hash(elemento.id)
        self.table[hash_index].append(elemento)
...
def get_minirating_info(players_ht, users_ht):
    with open('../data/minirating.csv', 'r') as file:
        ...
        for row in reader:
            ...
            users_ht.update_user(row[0], row[1], row[2])

            # Acha o index do jogador do id atual
            hash_index = players_ht._hash(row[1])

            # Percorre a lista de jogadores do id atual
            for jogador in players_ht.table[hash_index]:
                if jogador.id == row[1]:
        ...
        for jogador in players_ht.table[players_ht._hash("158023")]:
